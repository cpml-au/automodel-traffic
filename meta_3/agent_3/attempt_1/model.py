"""Incumbent correction augmented by a three-cell DEC convolution."""
...
def _incumbent_values(baseline_key: str, values):
    """Evaluate the selected meta-2 multiplier with fixed coefficients."""
...
def make_correction(baseline_key: str) -> Callable:
    """Return ``g_inc*exp(a*conv_3(rho,ones))`` for one fixed incumbent."""
    ...
    def correction(rho: C.CochainP0, parameters: Sequence[float]) -> C.CochainP0:
        ...
        values = _incumbent_values(baseline_key, rho.coeffs) * jnp.exp(a * conv_3)
...
def expression(baseline_key: str) -> str:
    return f"({INCUMBENT_EXPRESSIONS[baseline_key]})*exp(a*conv_3(rho,ones))"
