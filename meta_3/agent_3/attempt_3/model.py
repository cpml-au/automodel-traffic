"""Incumbent correction with level and contrast convolution features."""
...
def make_correction(baseline_key: str) -> Callable:
    """Return ``g_inc*exp(a*conv_3+b*(conv_3-3*conv_1))``."""
    ...
    def correction(rho: C.CochainP0, parameters: Sequence[float]) -> C.CochainP0:
        ...
        exponent = a * conv_3 + b * (conv_3 - 3.0 * conv_1)
        values = _incumbent_values(baseline_key, rho.coeffs) * jnp.exp(exponent)
...
def expression(baseline_key: str) -> str:
    return (
        ...
        "*exp(a*conv_3(rho,ones)+b*(conv_3(rho,ones)-3*conv_1(rho,ones)))"
    )
