"""Fixed meta-3 incumbents plus a ``conv_3 - 4*conv_1`` refinement."""
...
def make_correction(baseline_key: str) -> Callable:
    """Return the fixed incumbent times a fitted window-4 contrast factor."""
...
def expression(baseline_key: str) -> str:
    return (
        ...
        "*exp(a*(conv_3(rho,ones)-4*conv_1(rho,ones)))"
    )
